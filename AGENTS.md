...
This repository backs up my user-level Claude Code, Codex, shared-agent, and
Bastion config. It is kept in sync by an agent routine that runs both directions:
user config to repo (backup) and repo to user config (restore).
...
- the directory mapping (`dotclaude/` <-> `~/.claude/`, `dotcodex/` <-> `~/.codex/`,
  `dotagents/` <-> `~/.agents/`, and `dotbastion/` <-> Bastion's platform config
  directory)
...
`dotcodex/AGENTS.md`, and `dotagents/AGENTS.md`.
