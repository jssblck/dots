...
## The pattern: three files plus a thin cloud shell
...
| `.claude/bootstrap.mjs` | node | every session start and resume | local + cloud, all 3 OSes | cheap per-session prep (env vars, services, deps) |
| `.claude/settings.json` | n/a | n/a | repo config | wire `bootstrap.mjs` as a `SessionStart` hook |

Two deliberate choices. Keep both:

- **The cloud half is bash; the per-session half is Node.** The cloud setup
  script runs as root on Ubuntu, so bash plus `apt` is natural. The per-session
  half must run on macOS, Linux, and Windows, so it's Node: Claude Code itself
  runs on Node, so `node` is guaranteed present everywhere with no extra
  toolchain and no build step. Keep `bootstrap.mjs` to Node plus standard
  library only.
...
- the lint / format / test commands and the repo's branch and commit conventions.
...
rust, node 20/21/22, python plus uv, go, ruby, a JVM, docker plus compose, the
...
repo-agnostic. Node plus standard library only. Each step **self-gates**, so the
one file is correct both locally and in the cloud (no blanket "cloud only"
guard):
...
Open the environment dialog (cloud icon, then add/edit environment) and set the
...
- [`references/cloud-setup.sh`](references/cloud-setup.sh): idempotent toolchain-install template (gh worked example plus stub).
