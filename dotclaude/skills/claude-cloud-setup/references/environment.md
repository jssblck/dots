...
dialog in Claude Code on the web (cloud icon, then add/edit environment) and set
four fields.
...
`cloud-setup.sh`: no file is a clean skip and the session still starts, while a
file that exists still runs and surfaces real install failures. Keep the heavy
logic in `cloud-setup.sh`, not in this field, so it's versioned and reviewable.
...
- **Custom** keeps the default list and lets you add specific hosts. Switch to
  ...
  a runtime fetches from a host not on the Trusted list.
...
also needs a state backend plus cloud credentials the environment may not have).
To enable those, set Network access to **Custom**, keep the default list, and add
